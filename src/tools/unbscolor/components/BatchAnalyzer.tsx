    ...
    return (
        <div className="space-y-12">
            <div className="flex justify-between items-center mb-8 border-b border-gray-100 pb-4">
                <h2 className="font-mono text-xs font-bold text-gray-400 uppercase tracking-widest">{t.multiSlotMatchAnalysis}</h2>
                <div className="flex items-center gap-3">
                    <button onClick={onCopyAll} className="bg-black text-white px-6 py-2 font-mono text-[10px] font-bold uppercase tracking-widest hover:bg-gray-800 transition-all shadow-lg">{t.copyAllSlotsData}</button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                {batchColors.map((c, idx) => {
                    ...
                    return (
                        <div key={idx} className="bg-gray-50 p-6 rounded-[2rem] border border-gray-100 flex flex-col gap-6">
                            <div className="flex gap-4 items-center">
                                ...
                                <div className="flex flex-col">
                                    <input
                                        ...
                                        maxLength={7}
                                    />
                                    <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{getClosestColorName(c)}</span>
                                </div>
                                <div className="flex gap-2 ml-auto">
                                    <button
                                        onClick={() => onDownloadCard('svg', idx)}
                                        className="px-2 py-1 text-[10px] font-mono font-bold uppercase tracking-widest border border-gray-200 bg-white hover:border-black hover:text-black transition-all"
                            ...
                            <div className="space-y-1 font-mono text-[11px] text-gray-500 bg-white/50 p-5 rounded-xl border border-black/5 min-h-[140px]">
                            ...
                            <div className="p-4 bg-white rounded-2xl shadow-sm border border-black/5 space-y-4">
                                {settings.showPmsC && (
                                    <div className="flex items-center justify-between">
                                        <div className="flex flex-col">
                                            <span className="text-[9px] font-bold text-gray-300 uppercase">{t.pantoneBridgeC}</span>
                                ...
                                {settings.showPmsU && (
                                    <div className="flex items-center justify-between">
                                        <div className="flex flex-col">
                                            <span className="text-[9px] font-bold text-gray-300 uppercase">{t.pantoneBridgeU}</span>
                                ...
                                {settings.showPmsSolidC && (
                                    <div className="flex items-center justify-between">
                                        <div className="flex flex-col">
                                            <span className="text-[9px] font-bold text-gray-300 uppercase">{t.pantoneC}</span>
                                ...
                                {settings.showPmsSolidU && (
                                    <div className="flex items-center justify-between">
                                        <div className="flex flex-col">
                                            <span className="text-[9px] font-bold text-gray-300 uppercase">{t.pantoneU}</span>
                                        ...
                                        {strip.map((s, i) => (
                                            <div key={i} className="flex items-center gap-3 p-3 bg-white rounded-xl border border-black/5">
                                                ...
                                                <div className="flex flex-col">
                                                    ...
                                                    <span className="text-[10px] font-mono text-gray-400">{s.type}</span>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
