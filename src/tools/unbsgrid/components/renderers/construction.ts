...
import { hexToColor, PHI, clipLineToRect, type StyleConfig, type RenderContext } from './utils';

/**
 * Triangular Grid — three families of parallel lines at 0°, 60° and 120°,
 * producing equilateral triangles. Spacing is ~bounds.width / 12.
 */
export function renderTriangularGrid(
  ...
  const spacing = Math.max(4, bounds.width / 12);
  const cx = bounds.center.x;
  const cy = bounds.center.y;
  // Diagonal length of bounds — guarantees lines sweep the full rect when rotated.
  ...
  for (const angle of families) {
    // Perpendicular direction: we step lines along this vector.
    const perpDx = -Math.sin(angle);
    const perpDy = Math.cos(angle);
    // Number of lines on each side of center to cover the diagonal.
...
export function renderPolarGrid(
  ...
  for (let i = 1; i <= ringCount; i++) {
    ...
    c.strokeColor = i === ringCount ? color : dimColor;
  ...
  for (let i = 0; i < spokeCount; i++) {
    ...
    // Major axes (every 90°) brighter than minor spokes
    line.strokeColor = i % 3 === 0 ? color : dimColor;
    line.strokeWidth = style.strokeWidth * (i % 3 === 0 ? 1 : 0.7);
  }
